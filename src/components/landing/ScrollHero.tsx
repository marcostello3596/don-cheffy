...
function FeaturePanel({
  ...
  const start = 0.2 + (index / total) * 0.6;
  const peak = start + 0.06;
  const end = 0.2 + ((index + 1) / total) * 0.6;
  ...
  return (
    ...
    >
      <span className="text-5xl mb-5 block">{emoji}</span>
...
const ScrollHero = () => {
  ...
  const bubbleData = [
    { emoji: "🛒", left: "4%", top: "18%" },
    { emoji: "😩", right: "4%", top: "15%" },
    { emoji: "💸", left: "3%", top: "48%" },
    { emoji: "🤷", right: "3%", top: "52%" },
    { emoji: "📝", left: "6%", top: "78%" },
    { emoji: "🍽️", right: "6%", top: "80%" },
  ];
  ...
  return (
    <section ref={containerRef} id="hero" className="relative" style={{ height: "500vh" }}>
      <div className="sticky top-0 h-screen overflow-hidden" style={{ background: "var(--hero-gradient)" }}>
        {/* Phase 1: Hero headline */}
        <motion.div style={{ opacity: heroTextOpacity, y: heroTextY }} className="absolute inset-0 z-30 flex items-center justify-center">
            ...
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                <Button onClick={() => scrollTo("how-it-works")} variant="ghost" size="lg" className="rounded-full px-8 py-6 text-base gap-2">
                  {t.hero.discover[lang]} <ArrowDown className="h-4 w-4" />
                </Button>
              </div>
            </motion.div>
          </div>
        ...
        {/* Phase 2: Phone + Feature text */}
        <motion.div style={{ opacity: phoneOpacity, scale: phoneScale }} className="absolute inset-0 z-20 flex items-center justify-center">
          <div className="w-full max-w-6xl mx-auto px-6 sm:px-10">
            ...
            <div className="hidden lg:grid lg:grid-cols-2 lg:gap-16 items-center">
              ...
              <div className="flex justify-center">
                <PhoneWithScroll imageIndex={imageIndex} images={images} alts={t.features.map(f => f.alt[lang])} />
              </div>
            </div>
        ...
        {/* Scroll indicator */}
        <motion.div style={{ opacity: heroTextOpacity }} className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30">
          <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 1.5 }} className="text-muted-foreground">
...
/* ─── Phone mockup ─── */
function PhoneWithScroll({ imageIndex, images, alts }: { imageIndex: MotionValue<number>; images: string[]; alts: string[] }) {
  return (
...
function PhoneImage({ src, alt, index, imageIndex }: { src: string; alt: string; index: number; imageIndex: MotionValue<number> }) {
  const opacity = useTransform(imageIndex, (v: number) => (v === index ? 1 : 0));
  return (
    <motion.img src={src} alt={alt} className={`w-full block ${index > 0 ? "absolute inset-0" : ""}`} style={{ opacity }} loading={index === 0 ? "eager" : "lazy"} />
  );
}
  ...
  return (
    <div className="relative min-h-[100px]">
      {t.features.map((f, i) => {
        ...
        return (
          <MobileFeatureItem key={i} title={f.title[lang]} description={f.description[lang]} emoji={featureEmojis[i]} scrollProgress={scrollProgress} start={start} peak={peak} end={end} isLast={i === n - 1} />
        );
      })}
    </div>
  );
...
function MobileFeatureItem({ title, description, emoji, scrollProgress, start, peak, end, isLast }: { title: string; description: string; emoji: string; scrollProgress: MotionValue<number>; start: number; peak: number; end: number; isLast: boolean }) {
  const opacity = useTransform(scrollProgress, [start, peak, end - 0.06, end], [0, 1, 1, isLast ? 1 : 0]);
  return (
